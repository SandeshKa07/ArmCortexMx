 ...
 * In vector table, address of Default_handler will be stored,
 * so an exception to NMI will trigger the execution of Default_handler
 * Create it as weak, so programmer can implement in his source file, the real handler
 * using the same name.
 */
...
/* This is an initialised section which goes to .data section by default,
 * so compiler should be instructed to put this into user defined section.
 * using __attribute__((section)).  
 */
...
void Reset_handler(void){
	/* copy all .data section from flash to SRAM */
	
	/* Initialize the .bss section with zero */
	
	/* Call Init function of the standard library (printf, scanf, etc) */
